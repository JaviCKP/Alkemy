...
def test_normal_distribution_is_centered(sample, make_column):
    col = make_column("numeric")
    vals = sample(num(min=0, max=100, distribution="normal", mean=50, std=10), 2000, column=col)
...
def test_lognormal_is_right_skewed(sample, make_column):
    col = make_column("numeric")
    vals = sample(
        num(min=10, max=1000, distribution="lognormal", median=90, sigma=0.5),
        2000,
        column=col,
    )
...
def test_zipf_favors_the_low_end(sample, make_column):
    col = make_column("integer")
    vals = sample(num(min=1, max=10, distribution="zipf", s=1.5), 2000, column=col)
